...
const shortcuts = [
  { label: 'Your School', icon: Landmark, color: 'bg-blue-500' },
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.from('schools').select('*').eq('id', profile.school_id).maybeSingle().then(({ data }) => setSchool(data))
    supabase.from('profiles').select('*', { count: 'exact', head: true }).eq('school_id', profile.school_id).eq('status', 'active').then(({ count }) => setMemberCount(count ?? 0))
  }, [profile?.school_id, profile?.status, profile?.account_type])

  function handleShortcut(label) {
    if (label === 'Your School') {
      navigate(isSchoolMember(profile) ? '/school-profile' : '/schools')
      return
    }
    const item = shortcuts.find((s) => s.label === label)
    ...
    <div className="app-shell">
      <div className="bg-brand-navy text-white px-4 pt-4 pb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setSearchOpen((s) => !s)}><Search size={20} /></button>
          <button onClick={() => navigate('/notifications')} className="relative">
            <Bell size={20} />
            {unread > 0 ? <span className="absolute -top-1 -right-1 bg-red-500 rounded-full w-3.5 h-3.5 text-[9px] flex items-center justify-center">{unread > 9 ? '9+' : unread}</span> : null}
          </button>
        </div>
      </div>
      ...
      {searchOpen ? (
        <div className="px-4 py-2 bg-white border-b border-gray-100 flex items-center gap-2">
          <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search posts or people…" className="flex-1 border border-gray-200 rounded-full px-4 py-2 text-sm outline-brand-purple" />
      ...
      <div className="screen-scroll">
        <div className="flex gap-4 px-4 py-4 overflow-x-auto">
          {shortcuts.map(({ label, icon: Icon, color }) => (
            <button key={label} onClick={() => handleShortcut(label)} className="flex flex-col items-center gap-1 shrink-0">
              <span className={`w-14 h-14 rounded-full ${color} flex items-center justify-center`}><Icon size={22} className="text-white" /></span>
        ...
        <div className="px-4 mt-4 space-y-4">
          {loading ? (
            ...
            <div className="text-center text-gray-400 mt-8">
              {searchQuery ? <p>No posts match "{searchQuery}".</p> : (<><p>No posts yet.</p><p className="text-sm">Be the first to share something with your school.</p></>)}
            </div>
          ) : (
