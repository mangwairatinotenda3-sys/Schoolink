...
export default function CommunityDetail() {
  ...
  async function handleSend() {
    ...
    await supabase.from('community_messages').insert({ community_id: communityId, sender_id: user.id, sender_name: profile?.full_name || user.email, content, status })
    setSending(false)
  }
  ...
  async function handleFileChange(e) {
    ...
    if (!uploadError) {
      ...
      await supabase.from('community_messages').insert({
        community_id: communityId, sender_id: user.id, sender_name: profile?.full_name || user.email,
        media_url: data.publicUrl, media_type: 'file', content: file.name, status,
      })
    }
    setUploading(false)
  ...
  async function handleForwardSend(recipientId) {
    ...
    await supabase.from('messages').insert({
      sender_id: user.id, receiver_id: recipientId, content: forwarding.content,
      media_url: forwarding.media_url, media_type: forwarding.media_type,
    })
    setForwarding(null)
  ...
  return (
    <div className="app-shell">
      <div className="flex items-center border-b border-gray-100">
        <button onClick={() => navigate(`/communities/${communityId}/info`)} className="flex items-center gap-3 px-4 py-3 text-left flex-1">
      ...
      <div className="screen-scroll px-4 py-3 flex flex-col gap-2">
        {messages.length === 0 ? (
          ...
          messages.map((m) => (
            <div key={m.id}>
              <CommunityMessageBubble message={m} isMine={m.sender_id === user.id} onDeleted={handleDeleted} onForward={setForwarding} />
              {m.status === 'pending' ? (
                <div className={`text-[10px] flex items-center gap-2 mt-0.5 ${m.sender_id === user.id ? 'justify-end' : ''}`}>
                  ...
                  {isAdmin ? <button onClick={() => handleApproveMessage(m.id)} className="text-brand-purple font-medium flex items-center gap-1"><ShieldCheck size={11} /> Approve</button> : null}
                </div>
              ) : null}
            </div>
          ))
      ...
      {isMember ? (
        canPost ? (
          <div className="flex items-center gap-2 px-4 py-3 border-t border-gray-100">
            <button onClick={() => photoInputRef.current?.click()} disabled={uploading}><ImageIcon size={19} className={uploading ? 'text-gray-300' : 'text-brand-purple'} /></button>
            <input ref={photoInputRef} type="file" accept="image/*,video/*" onChange={handlePhotoChange} className="hidden" />
            <button onClick={() => fileInputRef.current?.click()} disabled={uploading}><Paperclip size={18} className={uploading ? 'text-gray-300' : 'text-gray-500'} /></button>
