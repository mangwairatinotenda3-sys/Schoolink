...
export default function Library() {
  ...
  async function searchExternal(q) {
    ...
    try {
      ...
      try {
        const ia = await fetch(`https://archive.org/advancedsearch.php?q=${sq}+AND+mediatype:texts&fl[]=identifier,title,creator,year&rows=10&output=json`).then(r=>r.json())
        ia.response?.docs?.forEach(d=>{
          results.push({
            title: d.title,
            author: d.creator || 'Unknown',
            year: d.year || '',
            cover: `https://archive.org/services/img/${d.identifier}`,
            file_url: `https://archive.org/details/${d.identifier}`, // NEVER 404
            source: 'Archive.org'
          })
        })
      } catch {}
      ...
      try {
        const g = await fetch(`https://gutendex.com/books/?search=${sq}`).then(r=>r.json())
        g.results?.slice(0,6).forEach(b=>{
          const pdf = b.formats['application/pdf'] || b.formats['text/html']
          if(pdf) results.push({
            title: b.title,
  ...
  async function handleAdd(){
    if(!form.title.trim()||!form.file_url.trim()){ setError('Add title and file'); return }
    ...
    <div className="flex-1 flex flex-col bg-background">
      ...
      <div className="px-4 pt-2 pb-2">
        <div className="relative">
          ...
          <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search..." className="w-full bg-card border border-border rounded-xl pl-9 pr-4 py-2.5 text-sm text-foreground outline-none focus:border-brand-purple" />
        </div>
        ...
        <div className="px-4 pb-2 mt-1">
          {showAddForm? (
            <div className="border border-border rounded-xl p-4 space-y-2 bg-card">
              ...
              <input value={form.title} onChange={e=>updateForm('title',e.target.value)} placeholder="Title *" className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground" />
              <input value={form.author} onChange={e=>updateForm('author',e.target.value)} placeholder="Author" className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground" />
              ...
              <input value={form.file_url} onChange={e=>updateForm('file_url',e.target.value)} placeholder="File link will appear here" className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground" />
        ...
        {query.trim().length>=2 && (
          <div className="pt-2">
            ...
            <div className="space-y-3">
              {externalResults.map((book,i)=>(
                <div key={i} className="bg-card border border-border/50 rounded-xl p-3 flex gap-3">
                  <div className="relative shrink-0">
                    <img src={book.cover||placeholder} onError={(e)=>{e.currentTarget.src=placeholder}} alt="" className="w-12 h-16 object-cover rounded-lg bg-muted" />
                    <span className="absolute -top-1.5 -right-1.5 text-[7px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">ONLINE</span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-sm text-foreground leading-tight line-clamp-2">{book.title}</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">{book.author} • {book.source}</p>
              ...
              {searchingExternal && <p className="text-center text-xs text-muted-foreground py-2">Searching...</p>}
            </div>
          </div>
        )}
      </div>
