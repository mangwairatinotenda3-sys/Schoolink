...
import { Send, Check, CheckCheck, UserX, X, CornerUpLeft, Mic, Square, Play } from 'lucide-react'
    ...
    <div className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
      <div className="max-w-[75%]">
        <div
          ...
          className={`rounded-2xl px-4 py-2 text-sm ${isMine ? 'bg-brand-purple text-white rounded-br-sm' : 'bg-gray-100 text-brand-navy rounded-bl-sm'}`}
        >
          {repliedTo ? (
            <div className={`text-xs border-l-2 pl-2 mb-1 opacity-80 ${isMine ? 'border-white/50' : 'border-brand-purple/50'}`}>{repliedTo.content}</div>
          ) : null}
        ...
        {showReactionBar ? (
          <div className="flex items-center gap-1 mt-1 bg-white border border-gray-100 rounded-full px-2 py-1 shadow w-fit">
            {QUICK_REACTIONS.map((emoji) => (
              <button key={emoji} onClick={() => { onReact(m.id, emoji); setShowReactionBar(false) }} className="text-base">{emoji}</button>
            ))}
  ...
  return (
    <div className="app-shell">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
        ...
        <button onClick={handleBlock} className="ml-1"><UserX size={16} className={isBlocked ? 'text-red-500' : 'text-gray-400'} /></button>
      </div>
      ...
      <div className="screen-scroll px-4 py-3 flex flex-col gap-3">
        {messages.map((m) => (
          <MessageBubble key={m.id} m={m} isMine={m.sender_id === user.id} reactions={reactions} onReact={handleReact} onReply={setReplyTo} allMessages={messages} />
        ))}
        <div ref={bottomRef} />
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-gray-100">
        <button
          ...
          disabled={isBlocked}
          className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${recording ? 'bg-red-500' : 'bg-gray-100'} disabled:opacity-60`}
        >
        ...
        />
        <button onClick={handleSend} disabled={sending || isBlocked || recording} className="w-10 h-10 rounded-full bg-brand-purple flex items-center justify-center shrink-0 disabled:opacity-60">
